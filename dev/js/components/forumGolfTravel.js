...
import {getGolfTravelCategory,  getGolfTravelCategoryObject, getGolfTravelCategoryDetails, 
  addNewGolfTravelCategory, addNewGolfTravelCategoryPost,
  addGolfTravelCategoryComment, searchGolfTravelCategoryPosts, searchCategory} from '../actions/forumGolfTravelAction';
import Spinner from 'react-spinner';
class ForumGolfTravel extends React.Component{
  constructor(props) {
    super(props);
    this.state={ postMsg : "",catMsg:"",replyMsg:"",catErr:"",
                  Category:Object.assign([],props.forumCourse),
                  categoryDetails:Object.assign({}, props.selectedGolfTravelCategory), 
                  Details:Object.assign({}, props.selectedGolfTravelCategory), ajaxCallInProgress:false
          };
 ...
 componentDidMount() {
    ...
    componentWillMount(){
       this.setState({ajaxCallInProgress:true});
        this.props.getGolfTravelCategory(this.props.activeUser.token).then(()=>{
            this.setState({Category:this.props.forumCourse.getGolfTravelCategory});
         ...
         if(_.size(this.state.categoryDetails)==0 && _.size(this.props.getGolfTravelCategory)>0){
                this.showCategoryDetails(this.props.forumCourse.getGolfTravelCategory[0].id);
               
        }
    }
   componentWillReceiveProps(nextProps){
         if(this.props.getGolfTravelCategory!=nextProps.getGolfTravelCategory)
        {
            this.setState({Category:nextProps.forumCourse.getGolfTravelCategory});
        }
       /* if(_.size(this.state.categoryDetails)==0 && _.size(this.state.Category)>0){
                this.showCategoryDetails(this.state.Category[0].id);
          }*/

        if(this.props.selectedGolfTravelCategory!=nextProps.selectedGolfTravelCategory){
                this.setState({categoryDetails:nextProps.selectedGolfTravelCategory.Category, 
                  Details:nextProps.selectedGolfTravelCategory.CatDetails});
        }
     ...
     showCategoryDetails(id){
      ...
      this.props.getGolfTravelCategoryDetails(id, this.props.activeUser.token).then(()=>{
              this.setState({Details:this.props.selectedGolfTravelCategory.CatDetails,catErr:""});
              
               this.props.getGolfTravelCategoryObject(id, this.props.activeUser.token).then((id)=>{
        
          this.setState({categoryDetails:this.props.selectedGolfTravelCategory.Category});
     ...
     getCategoryConversation(){
      this.props.getGolfTravelCategoryObject(this.props.selectedGolfTravelCategory.CatDetails.id, this.props.activeUser.token).then((id)=>{
        
          this.setState({categoryDetails:this.props.selectedGolfTravelCategory.Category,postMsg : ""});
     ...
     onCategorySearch(e){
          if(e.which==13)
          {
            this.props.searchCategory(this.props.activeUser.token, e.target.value).then(()=>{
                  ...
                  this.setState({Category:this.props.selectedGolfTravelCategory.SearchList});
      ...
      onPostSearch(e){
          ...
          if(e.which==13)
          {
            this.props.searchGolfTravelCategoryPosts(this.props.activeUser.token, this.props.selectedGolfTravelCategory.CatDetails.id, e.target.value).then(()=>{
                  this.setState({categoryDetails:this.props.selectedGolfTravelCategory.CategorySearchList});
      ...
      this.props.addNewGolfTravelCategory(this.props.activeUser.token, name.value).then(()=>{
       this.props.getGolfTravelCategory(this.props.activeUser.token).then(()=>{
            this.setState({Category:this.props.forumCourse.getGolfTravelCategory,catMsg : ""});
    ...
    addPost(){
      ...
      let subject_line = document.getElementById('PostInput');
        console.log("this.props.selectedCategory.id",this.props.selectedGolfTravelCategory.CatDetails.id);

      this.props.addNewGolfTravelCategoryPost(this.props.selectedGolfTravelCategory.CatDetails.id, this.props.activeUser.token, subject_line.value).then(()=>{
    ...
    Comment(id){
      ...
      this.props.addGolfTravelCategoryComment(this.props.selectedGolfTravelCategory.CatDetails.id, id , this.props.activeUser.token, body).then(()=>{
            ...
            {(this.state.ajaxCallInProgress)?(<div className="mt25pc"><Spinner /></div>):(<div className="row">
              <div className="headerContent col-sm-12">
                <div className="courseHeader">
                  <h3 className="header"><span className="glyphicon glyphicon-chevron-left arrowChevron" onClick={this.onforumclick.bind(this)}/>GOLF TRAVEL</h3>
                </div>
              </div>
...
function mapStateToProps(state) {
    return {
        ...
        selectedGolfTravelCategory:state.selectedGolfTravelCategory
...
function matchDispatchToProps(dispatch){
    return bindActionCreators({getGolfTravelCategory,  getGolfTravelCategoryObject, getGolfTravelCategoryDetails, 
  addNewGolfTravelCategory, addNewGolfTravelCategoryPost,
  addGolfTravelCategoryComment, searchGolfTravelCategoryPosts, searchCategory}, dispatch);


}

export default  connect(mapStateToProps, matchDispatchToProps)(ForumGolfTravel);